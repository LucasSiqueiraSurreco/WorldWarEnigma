<template>
  <header class="column is-full has-text-centered mt-2 mb-5">
    ...
    <div class="description">
      <p>
        ...
        military during World War II. It is not a perfect replica of the original machine, but it is close enough to be
        useful for learning about it. It is a three-rotor machine, with a reflector and a plugboard. The rotors are
        interchangeable, as well as the reflector.
      </p>

      <p>
        I used this
        <a href="https://www.cryptomuseum.com/crypto/enigma/index.htm" target="_blank">excellent resource</a> to learn
        about the machine and to build this simulator, the logo is also a courtesy of them. And the code is heavily
        inspired on the professor <a href="http://github.com/mikepound">Mike Pound's</a> version of the Enigma machine,
        which you can find <a href="https://github.com/mikepound/enigma" target="_blank">here</a>.
      </p>

      <p>
        You can find the base code of this project at
        <a href="https://github.lsantos.dev/enigmajs" target="_blank">Lucas Santos</a> github with all the details and
        instructions.
      </p>
      <p>
        I coded the functionality of the reflectors, rotors and did minor changes, giving then life to the encryption
        machine
      </p>
    </div>
  </header>
  ...
  <section
    class="lampboard is-flex is-flex-direction-column is-align-items-center is-align-content-space-between is-justify-content-space-around has-text-centered"
  >
    <Lampboard :switch-on="switchedOnLetter" />
  </section>

  <section
    class="keyboard is-flex is-flex-direction-column is-align-items-center is-align-content-space-between is-justify-content-space-around has-text-centered mb-5"
  >
    <h1>Input</h1>
  ...
  <section class="steps mb-5">
    <h1 class="has-text-centered">
      <a @click="showSteps = !showSteps">{{ showSteps ? 'Hide' : 'Show' }} encoding steps</a>
    </h1>
...
const handleKeyPress = (keyChar: AllowedAlphabet) => {
  const {
    rotors: { LEFT: leftRotor, MIDDLE: middleRotor, RIGHT: rightRotor },
    reflector
  } = state
  addStep(`-------- ROTATION FOR KEY ${keyChar} --------`)
  addStep(
    `Rotors before rotation: ${toChar(leftRotor.position)} ${toChar(middleRotor.position)} ${toChar(
      rightRotor.position
    )}`
  )
  ...
  addStep(
    `Middle rotor (${toChar(middleRotor.position)}): ${toChar(rightRotorToMiddle)} -> ${toChar(middleRotorToLeft)}`
  )
  ...
  addStep(
    `Middle rotor (${toChar(middleRotor.position)}): ${toChar(leftRotorToMiddle)} -> ${toChar(middleRotorToRight)}`
  )
  // perform the second plugboard substitution
  const rightRotorToPlugboard = rightRotor.encodeReturn(middleRotorToRight)
  addStep(
    `Right rotor (${toChar(rightRotor.position)}): ${toChar(middleRotorToRight)} -> ${toChar(rightRotorToPlugboard)}`
  )
  // final output
  ...
  addStep(
    `Rotors after rotation: ${toChar(leftRotor.position)} ${toChar(middleRotor.position)} ${toChar(
      rightRotor.position
    )}`
  )
