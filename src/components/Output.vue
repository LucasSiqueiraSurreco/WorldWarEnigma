<template>
  <div
    class="header is-flex is-flex-direction-row is-align-content-center is-justify-content-center is-align-items-center"
  >
  ...
  <section class="columns output-wrapper has-text-centered is-family-code">
    <div class="column output-text">
      <textarea disabled :value="splitInBlocks(state.output)"> </textarea>
    </div>
  </section>
</template>
...
const splitInBlocks = (text: string) => {
  outputText.value = chunk(text.toUpperCase().split(''), 5)
